...
if not exists (select * from INFORMATION_SCHEMA.TABLES where TABLE_NAME='ServiceWarning') 
begin 
	...
	(
		[ServiceWarningID] ASC
	)WITH (PAD_INDEX  = OFF, STATISTICS_NORECOMPUTE  = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS  = ON, ALLOW_PAGE_LOCKS  = ON) ON [PRIMARY]
	) ON [PRIMARY]
	
	insert into ServiceWarning(ServiceID, [Description], ValidFrom, ValidTo)
	select ServiceID, Warning, '2000.01.01', '3000.01.01'
	from Service where Warning is not null and Warning <> ''
end
GO

GO
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[ServiceWarning_Ins]') and OBJECTPROPERTY(id, N'IsProcedure') = 1) drop procedure [dbo].[ServiceWarning_Ins]
GO
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[ServiceWarning_Upd]') and OBJECTPROPERTY(id, N'IsProcedure') = 1) drop procedure [dbo].[ServiceWarning_Upd]
GO
...
if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[ServiceWarning_Del]') and OBJECTPROPERTY(id, N'IsProcedure') = 1) drop procedure [dbo].[ServiceWarning_Del]
GO

CREATE PROCEDURE [dbo].[ServiceWarning_Del]
	@ServiceWarningID int
AS
DELETE FROM [dbo].[ServiceWarning]
WHERE
	[ServiceWarningID] = @ServiceWarningID
GO

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[ServiceWarning_Sel_ByID]') and OBJECTPROPERTY(id, N'IsProcedure') = 1) drop procedure [dbo].[ServiceWarning_Sel_ByID]
GO

CREATE PROCEDURE [dbo].[ServiceWarning_Sel_ByID]
	@ServiceWarningID int
AS
SET NOCOUNT ON
SELECT
	...
	[ValidFrom],
	[ValidTo]
FROM [dbo].[ServiceWarning]
WHERE
	[ServiceWarningID] = @ServiceWarningID
GO

if exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[ServiceWarning_Sel_All]') and OBJECTPROPERTY(id, N'IsProcedure') = 1) drop procedure [dbo].[ServiceWarning_Sel_All]
GO

CREATE PROCEDURE [dbo].[ServiceWarning_Sel_All]
AS
SET NOCOUNT ON
SELECT
	[ServiceWarningID],
