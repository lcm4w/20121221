...
BEGIN TRANSACTION
GO
if ((select VersionNumber from AppSettings) <> '2011.02.02')
...
select
		...
		isnull(itinCcy.Symbol, sysCcy.Symbol) as ItineraryCurrencyFormat,
...
select
		...
		isnull(itemCcy.Symbol, sysCcy.Symbol)as CurrencyFormat,  
		item.CurrencyRate,
		pric.Net,
		pric.Gross,
		item.NumberOfDays,
...
GO
UPDATE [dbo].[AppSettings] SET [VersionNumber]='2011.02.10'
GO
