...
namespace Travelmesh
{
    class DataSync
    {
        internal static void SyncSupplier(SupplierSet.SupplierRow supplier)
        {
            ...
            supplier.ImportID = ApiWrapper.ApiRequest<int>("/supplier", exported ? "PUT" : "POST", ApiWrapper.ConvertToJson(sup), "application/json; charset=utf-8");
            if (!exported)
        ...
        internal static void SyncService(SupplierSet.ServiceRow service)
        {
            ...
            service.ImportID = ApiWrapper.ApiRequest<int>("/service", exported ? "PUT" : "POST", ApiWrapper.ConvertToJson(srv), "application/json; charset=utf-8");
            if (!exported)
        ...
        private static void SyncRates(SupplierSet.ServiceRow service, int? forOptionId)
        {
            ...
            {
                forOptionId = ApiWrapper.ApiRequest<int>("/option", "POST", ApiWrapper.ConvertToJson(option), "application/json; charset=utf-8");
                ...
                foreach (var x in option.Rates)
                {
                    ...
                    if (overridePrice.HasValue) dict.Add("OverridePrice", forOptionId); // skip null
                    if (overridePercent.HasValue) dict.Add("OverridePercent", forOptionId);

                    baseOption.ImportID = ApiWrapper.ApiRequest<int>("/rate", exported ? "PUT" : "POST", ApiWrapper.ConvertToJson(dict), "application/json; charset=utf-8");
                    if (!exported)
                        TourWriter.Info.Services.DatabaseHelper.ExecuteScalar(string.Format(
                            "update [Option] set ImportID = {0} where OptionID = {1}", baseOption.ImportID, x.OptionID));
