...
namespace TourWriter.Services
{
    static class DataSyncUi
    {
        ...
        public static void AddExportHooks(this UltraGrid grid, SupplierSet supplierSet, params UltraGrid[] childGrids)
        {
            OnGridInitializeLayout(grid, supplierSet);
            grid.InitializeRow += (sender, e) => OnGridInitializeRow(e.Row, supplierSet);
            grid.AfterCellListCloseUp += (sender, e) => OnGridCellListCloseUp(e, supplierSet, childGrids);
        ...
        private static void OnEntitySyncClick(CellEventArgs e, SupplierSet supplierSet, params UltraGrid[] childGrids)
        {
            ...
            finally
            {
                ...
                row.Band.Layout.Grid.Rows.Refresh(RefreshRow.FireInitializeRow);
                foreach(var grid in childGrids) grid.Rows.Refresh(RefreshRow.FireInitializeRow);
            }
        }
        
        private static void OnEntityUnsyncClick(CellEventArgs e)
        {
            if (e.Cell.Column.Key != "Export") return;
        ...
        static void OnGridCellListCloseUp(CellEventArgs e, SupplierSet supplierSet, params UltraGrid[] childGrids)
        {
            ...
            if (result == "Sync")
                OnEntitySyncClick(e, supplierSet, childGrids);
            if (result == "Remove")
                OnEntityUnsyncClick(e);
            e.Cell.Value = null;
        }
