...
namespace TourWriter.Modules.ItineraryModule.ReportControls
{
	...
	/// </summary>
	public class SupplierMessage : System.Windows.Forms.UserControl
	{
		#region Designer
		...
		private void InitializeComponent()
		{
            ...
            this.chkIncEmptyBookings.TabIndex = 79;
		...
		private TourWriter.Reports.Itinerary.SupplierMessage.ReportType reportType;
		
		
		public SupplierMessage(TourWriter.Reports.Itinerary.SupplierMessage.ReportType reportType,
			ItinerarySet itinerarySet, AgentSet agentSet, UserSet userSet)
		{
			InitializeComponent();
		...
		private void btnReport_Click(object sender, System.EventArgs e)
		{
			...
			try
			{
				...
				// create batch report				
				ArrayList reportArray = BuildSupplierMessageReportArray(purchaseLineList.Split(','));
				
				// open in viewer				
				Modules.ReportViewer.ReportViewerForm viewer;
				viewer = new Modules.ReportViewer.ReportViewerForm(
					itinerarySet, 
					(DataDynamics.ActiveReports.ActiveReport[])
						reportArray.ToArray(typeof(DataDynamics.ActiveReports.ActiveReport)));
		...
		private ArrayList BuildSupplierMessageReportArray(string[] purchaseLineArray)
		{
			ArrayList reportArray = new ArrayList(purchaseLineArray.Length);

			for(int i = 0; i < purchaseLineArray.Length; i++)
			{
				if(string.IsNullOrEmpty(purchaseLineArray[i].Trim()))
					continue;

			    int purchaseLineID = int.Parse(purchaseLineArray[i]);
                
			    ItinerarySet.PurchaseLineRow purchaseLineRow
                    = itinerarySet.PurchaseLine.FindByPurchaseLineID(purchaseLineID);
                if (purchaseLineRow == null)
                    continue;

				//-------------------------------------------------------------------------------------
                 DataDynamics.ActiveReports.ActiveReport tempReport = new Reports.Itinerary.SupplierMessage(
					reportType, purchaseLineID, itinerarySet, agentSet, Cache.UserSet, txtFooter.Text, Cache.ToolSet);			
				//-------------------------------------------------------------------------------------
				//tempReport = new TourWriter.Reports.Itinerary.SupplierMessage2(
				//	TourWriter.Reports.Itinerary.SupplierMessage2.ReportType.Request, 
				//	purchaseLineID, itinerarySet, agentSet, TourWriter.Global.Cache.UserSet, txtFooter.Text, chkShowPricing.Checked);
				//-------------------------------------------------------------------------------------												

                // add contact info
                int supplierID = purchaseLineRow.SupplierID;
                ItinerarySet.SupplierLookupRow supplierRow = itinerarySet.SupplierLookup.FindBySupplierID(supplierID);		

				tempReport.Fields.Add("SupplierID");
				tempReport.Fields.Add("SupplierName");
				tempReport.Fields.Add("EmailTo");
				tempReport.Fields.Add("FaxTo");

				tempReport.Fields["SupplierID"].Value = supplierRow.SupplierID;
				tempReport.Fields["SupplierName"].Value = supplierRow.SupplierName;
				tempReport.Fields["EmailTo"].Value = !supplierRow.IsEmailNull() ? supplierRow.Email : "";
				tempReport.Fields["FaxTo"].Value = !supplierRow.IsFaxNull() ? supplierRow.Fax : "";
				
				reportArray.Add(tempReport);
			}
			return reportArray;
		}
